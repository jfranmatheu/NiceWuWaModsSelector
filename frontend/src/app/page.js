...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Listen for in-game mode changes
    const handleInGameModeChange = (event) => {
      setIsInGameMode(event.detail.isInGameMode);
      if (event.detail.isInGameMode) {
        setSelectedCategory("Characters");
        setContext("installed");
      }
    };

    window.addEventListener('ingame-mode-change', handleInGameModeChange);
    return () => window.removeEventListener('ingame-mode-change', handleInGameModeChange);
  }, []);
